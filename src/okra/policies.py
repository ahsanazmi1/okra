...
class CreditProfile(BaseModel):
    ...
    credit_history_months: Optional[int] = Field(None, ge=0, description="Credit history length in months")
...
class CreditPolicies:
    ...
    def evaluate_credit_request(cls, request: CreditRequest) -> CreditQuote:
        ...
        if request.amount < cls.MIN_LOAN_AMOUNT:
            reasons.append(f"Requested amount ${request.amount} below minimum ${cls.MIN_LOAN_AMOUNT}")
            return cls._create_declined_quote(request, reasons)
            
        if request.amount > cls.MAX_LOAN_AMOUNT:
            reasons.append(f"Requested amount ${request.amount} exceeds maximum ${cls.MAX_LOAN_AMOUNT}")
            return cls._create_declined_quote(request, reasons)
        ...
        if profile.annual_income and profile.annual_income < cls.MIN_ANNUAL_INCOME:
            reasons.append(f"Income ${profile.annual_income} below minimum ${cls.MIN_ANNUAL_INCOME}")
            return cls._create_declined_quote(request, reasons)
        
        # Check debt-to-income ratio
        if profile.debt_to_income_ratio and profile.debt_to_income_ratio > cls.MAX_DTI_RATIO:
            reasons.append(f"DTI ratio {profile.debt_to_income_ratio:.2%} exceeds maximum {cls.MAX_DTI_RATIO:.2%}")
            return cls._create_declined_quote(request, reasons)
        ...
        monthly_payment = cls._calculate_monthly_payment(credit_limit, monthly_rate, request.term_months)
    ...
    @classmethod
    def _calculate_monthly_payment(cls, principal: Decimal, monthly_rate: Decimal, months: int) -> Decimal:
    ...
    def list_policies(cls) -> Dict[str, any]:
        ...
        return {
            ...
            "rate_tiers": [
                {"min_score": score, "apr": float(apr)} 
                for score, apr in cls.RATE_TIERS
            ]
        }
